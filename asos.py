...
ADMIN_ID = 7861165622  # @userinfobot orqali olingan o'zingizning ID raqamingiz
...
async def start_cmd(message: types.Message):
    await users_col.update_one(
        ...
        {'$setOnInsert': {'balance': 0, 'total_deposited': 0, 'username': message.from_user.username}},
        upsert=True
    )
    await message.answer("Xush kelibsiz! Kerakli bo'limni tanlang:", reply_markup=main_menu)

@dp.message(F.text == "🚀 Buyurtma berish")
async def buy_order(message: types.Message):
...
async def show_services(message: types.Message):
    cat = message.text
    # Bazadan kategoriya bo'yicha xizmatlarni qidirish
    services = await services_col.find({"category": {"$regex": cat, "$options": "i"}}).limit(20).to_list(length=20)
    
    if not services:
        return await message.answer(f"😔 {cat} bo'limida hozircha xizmatlar yo'q.")
...
async def srv_select(callback: types.CallbackQuery, state: FSMContext):
    s_id = callback.data.split("_")[1]
    service = await services_col.find_one({'id': s_id})
    
    if not service:
        return await callback.message.answer("❌ Xatolik: Xizmat topilmadi.")
...
async def my_account(message: types.Message):
    ...
    text = (f"👤 ID: `{message.from_user.id}`\n"
            f"💵 Balans: {user.get('balance', 0):,.2f} so'm\n"
            f"💰 Jami kiritilgan: {user.get('total_deposited', 0):,.2f} so'm")
...
async def admin_stats(message: types.Message):
    ...
    o_count = await orders_col.count_documents({})
    ...
    await message.answer(f"📊 **Statistika:**\n\n👥 Foydalanuvchilar: {u_count}\n📦 Buyurtmalar: {o_count}\n💰 Jami tushum: {total_money:,.0f} so'm")

@dp.message(F.text == "🔄 Xizmatlarni yangilash")
async def update_services(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(SMM_API_URL, data={'key': SMM_API_KEY, 'action': 'services'}) as r:
            ...
            if isinstance(data, list):
                await services_col.delete_many({'manual': {'$ne': True}}) # Faqat qo'lda qo'shilmaganlarni o'chirish
                for s in data:
                ...
                await message.answer(f"✅ Tayyor! {len(data)} ta xizmat yuklandi.")
...
async def add_bal_final(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ {data['uid']} hisobiga {amount} qo'shildi.")
...
async def m_price(message: types.Message, state: FSMContext):
    ...
    await message.answer("SMM Panel Service ID (API ID):")
...
async def m_final(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ Qo'lda qo'shildi!")
    await state.clear()
